...
namespace AdlsSDKGettingStarted
{
    public class Program
    {
        ...
        private static string applicationId = "";     // Also called client id
        private static string clientSecret = "";
        private static string tenantId = "";
        private static string adlsAccountFQDN = "";   // ful
    ...
    */
       ...
       async static void readFile(AdlsClient adlsclient)
        {
            ...
            {
                using (HttpResponseMessage response = await hclient.GetAsync("https://raw.githubusercontent.com/LearnWebCode/json-example/master/animals-3.json"))
                {
                    using (HttpContent content = response.Content)
                    {
                        ...
                        string fileName = "/Test/testFilename1.txt";
                        // Create a file - automatically creates any parent directories that don't exist
                        using (var streamWriter = new StreamWriter(adlsclient.CreateFile(fileName, IfExists.Overwrite)))
                        {
